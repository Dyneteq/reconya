...
func scanNetworkQuick(networkCIDR string) {
	addScanLog("Running nmap scan...")

	cmd := exec.Command("nmap", "-sn", "-T4", "-oG", "-", networkCIDR)
	output, err := cmd.Output()
	if err != nil {
		addScanLog("nmap failed: %v, using ping sweep", err)
	...
	for _, line := range lines {
		...
		trimmed := strings.TrimPrefix(line, "Host: ")
		...
		addScanLog("Found host: %s %s", ip, hostname)
	}

	addScanLog("nmap found %d hosts, fetching ARP info...", hostsFound)
	getARPInfo()
}
