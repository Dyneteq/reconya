...
The topology map itself (`network-viz.js`) is a `<canvas>` (not SVG), fed by whatever the devices poll already fetched, there's no dedicated topology endpoint. Two layouts: TREE (gateway → core switch → one band per network sized by slot capacity → hosts in rows) and RADIAL (one ring per network around a center gateway). Node shape encodes role (square = infrastructure, triangle = unidentified, circle = normal host) and fill color encodes status (green = online, dim green = idle, dark = offline, amber = unidentified). New nodes fly out from the gateway point over ~900ms when they first appear; the canvas runs its own animation loop independent of the poll cadence, so motion stays smooth between data refreshes.

**Layout is stabilized against per-poll churn, not recomputed fresh from the live device list each time.** Each device earns a persistent slot within its network the first time it's seen (`assignSlots`, keyed by device ID) and keeps that slot, and its row/column or ring angle, for as long as it keeps appearing; a device dropping out (offline, filtered, deleted) frees its slot for reuse but does not shift its neighbours into the gap. A network's band height (TREE) or ring share (RADIAL) is driven by an eased "slot capacity" that trends toward the live device count rather than snapping to it, so one host flapping online/offline no longer reshuffles or resizes bands/rings it has nothing to do with. This is the frontend half of the map-stability fix; the backend half (fewer status flips reaching the client at all) is the two-tier presence engine, tracked separately.
...
**The OVERVIEW tab also shows DISCOVERY and FIRST SEEN**, read-only rows sourced from `discovery_method`/`first_seen_at` (not editable, no PUT endpoint touches them). FIRST SEEN is distinct from the `created_at`-derived value it used to show; see [01-device-identification.md](01-device-identification.md#long-term-tracking-first-seen--discovery-method).
